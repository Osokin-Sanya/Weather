...
function getCurrentWeatherData(data) {
  fetch(
    `https://api.openweathermap.org/data/2.5/weather?q=${inputCity.value}&lang=ru&units=metric&appid=${apiKey}`
  )
    .then((res) => res.json())
    .then((currentData) => {
    ...
    .catch(function (error) {
      if (error) {
        ...
        $(".weather-today").classList.add("noCity");
        ...
        if (inputCity.value == "Припять") {
          $(
            ".weather-today"
          ).innerHTML = `Ты бы еще консервных банок насобирал...`;
...
function getWeatherForecast() {
  ...
  $(".weather-today").classList.remove("noCity");
  $(".buffer-zone").style.display = "none";
  $(".current-wrapper").style.display = "none";
  $(".weather-forecast").style.display = "none";

  $(".weather-today").innerHTML = `<div class="loader"></div>`;

  fetch(
    `https://api.openweathermap.org/data/2.5/forecast?q=${inputCity.value}&lang=ru&units=metric&appid=${apiKey}`
  )
    .then((res) => res.json())
    .then((data) => {
...
function createCarddataCurrentWeather(data, currentData) {
  ...
  const Night = data.list.filter((nighttime) =>
    nighttime.dt_txt.includes("21:00:00")
  );
...
function futureWeatherData(data) {
  ...
  const morningData = data.list.filter((daytime) =>
    daytime.dt_txt.includes("18:00:00")
  );
  const serialNumberDay = morningData.map(mapCb);

  const nightData = data.list.filter((nighttime) =>
    nighttime.dt_txt.includes("21:00:00")
  );
  const serialNumberNight = nightData.map(mapCb);

  const formatterLong = new Intl.DateTimeFormat("ru-RU", { weekday: "long" });
  const formatterShort = new Intl.DateTimeFormat("ru-RU", { weekday: "short" });
  let formatter;
  ...
  for (let i = 0; i < data.list.length; i++) {
    const date = new Date(
      morningData[serialNumberDay.indexOf(serialNumberDay[i])].dt_txt
    );
    ...
    const filterDays = data.list.filter((daytime) =>
      daytime.dt_txt.includes(today)
    );
    ...
    cardDay.innerHTML = `
        ...
        <img src='http://openweathermap.org/img/wn/${
          filterDays[i].weather[0].icon
        }@2x.png'>
        <div class="temp">
        День - ${
          morningData[serialNumberDay.indexOf(serialNumberDay[i])].main.temp
        } °C
        </div>
        <div class="temp">
        Ночь - ${
          nightData[serialNumberNight.indexOf(serialNumberDay[i])].main.temp
        } °C
        </div>`;
      ...
      filterDays.forEach((day) => {
        ...
        $(".parameters-wraper").innerHTML += `
                    <div class="parameters-item">  
                        <p class="parameter-date">${day.dt_txt.slice(
                          10,
                          16
                        )}</p>
                        <p class="img"><img src='http://openweathermap.org/img/wn/${
                          day.weather[0].icon
                        }@2x.png'</p>
                        ...
                        <p class="parameter-feels_like">${
                          day.main.feels_like
                        }℃</p>
                        <p class="parameter-pressure">${(
                          day.main.pressure / 1.333
                        ).toFixed()} мм рт.ст</p>
...
function dataCurrentWeather(data, currentData) {
  $(".weather-today").addEventListener("click", () => {
    ...
    $(".pressure-item").innerHTML =
      (currentData.main.pressure / 1.333).toFixed() + " мм рт.ст.";
...
function autocomplete(inp, arr) {
  ...
  inp.addEventListener("input", () => {
    ...
    const showСities = arr.filter((elem) =>
      elem.toLowerCase().startsWith(inputValue.toLowerCase())
    );
  ...
  inp.addEventListener("keydown", (event) => {
    ...
    if (event.keyCode == KEY_DOWN) {
      ...
      if ($(`.autocomplete-item-hover`)) {
        $$(".autocomplete-item")[selectedIndex - 2].classList.remove(
    ...
    if (event.keyCode == KEY_UP) {
      ...
      $$(".autocomplete-item")[selectedIndex].classList.remove(
        "autocomplete-item-hover"
      );
    }
