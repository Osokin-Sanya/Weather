...
function autocomplete(inp, arr) {
    ...
    wrapperCity.className = 'autocomplete-items'
    ...
    inp.addEventListener('input', () => {
        ...
        for (let i = 0; i < showСities.length; i++) {
            const SearchCite = document.createElement('div')
            SearchCite.innerHTML = showСities[i]
            if (x.length > 0) {
                wrapperCity.appendChild(SearchCite)
            }
            SearchCite.addEventListener('click', () => {
                inp.value = SearchCite.textContent
                wrapperCity.innerHTML = '';
            })
        }
